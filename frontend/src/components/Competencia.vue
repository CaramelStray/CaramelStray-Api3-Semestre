            ...
            <header class="card__header">
              ...
              <span v-else class="card__added-icon" title="Já selecionada">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-5 w-5">
                  <path
                    fill-rule="evenodd"
                    d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12Zm13.36-1.814a.75.75 0 1 0-1.06-1.06l-3.25 3.25-1.5-1.5a.75.75 0 0 0-1.06 1.06l2 2a.75.75 0 0 0 1.06 0l3.75-3.75Z"
                    clip-rule="evenodd"
                  />
                </svg>
              </span>
...
const API_FUNCIONARIO_COMP_URL = computed(
  () => `http://localhost:8080/api/funcionario/${FUNCIONARIO_ID_ALVO.value}/competencias`
)
...
async function loadPerfilAlvo() {
  try {
    const response = await axios.get(
      `http://localhost:8080/api/funcionario/${FUNCIONARIO_ID_ALVO.value}`
    )
...
function add(comp) {
  if (!isSelected(comp.id)) selected.value.push(comp)
}
...
const isSelfEdit = computed(
  () =>
    !!(
      USUARIO_LOGADO_ID.value &&
      FUNCIONARIO_ID_ALVO.value &&
      USUARIO_LOGADO_ID.value === FUNCIONARIO_ID_ALVO.value
    )
)
  ...
  try {
    ...
    if (error.response) {
      if (error.response.status === 403)
        message =
          'Ação não autorizada. Você só pode alterar suas próprias competências ou de subordinados na mesma área.'
...
async function createCompetencia() {
  ...
  try {
    ...
    alert(`Falha ao criar competência: ${message}`)
  } finally {
...
onMounted(() => {
  Promise.all([loadCompetencias(), loadPerfilAlvo()])
})
</script>
...
.chip__dot[data-variant='Comportamental'] {
  background: #6c9cff;
}
.chip__dot[data-variant='Técnica'] {
  background: #38b48e;
}
.chip__dot[data-variant='Negócios'] {
  background: #f7b733;
}
.chip__dot[data-variant='RH'] {
  background: #b381f0;
}
.chip__dot[data-variant='Idiomas'] {
  background: #ef6f6c;
}
.chip__dot[data-variant='Outros'] {
  background: #a0aec0;
}
.chip__dot[data-variant='Erro'] {
  background: #f56530;
}
...
.save-button {
  ...
  box-shadow: 0 4px 8px rgba(56, 46, 206, 0.2);
  font-size: 15px;
}
...
.save-button:disabled {
  background: var(--muted, #eef3f7);
  color: var(--text, #142032);
  cursor: not-allowed;
  opacity: 0.6;
  box-shadow: none;
}
...
.create__button {
  ...
  font-weight: 600;
  font-size: 14px;
  box-shadow: 0 4px 8px rgba(56, 46, 206, 0.18);
  transition: background 0.2s, box-shadow 0.2s, opacity 0.2s;
}

.create__button:hover:not(:disabled) {
  background: #2a1fb8;
  box-shadow: 0 6px 12px rgba(56, 46, 206, 0.25);
}
...
.card:hover {
  ...
  box-shadow: 0 10px 20px rgba(18, 53, 70, 0.07);
}
.card:active {
  cursor: grabbing;
}
...
.pill[data-variant='Comportamental'] {
  color: #4a6fcc;
  background: #fff;
}
.pill[data-variant='Técnica'] {
  color: #2a7d68;
  background: #fff;
}
.pill[data-variant='Negócios'] {
  color: #b7791f;
  background: #fff;
}
.pill[data-variant='Idiomas'] {
  color: #cc5452;
  background: #fff;
}
.pill[data-variant='Outros'] {
  color: #4a5568;
  background: #fff;
}
.pill[data-variant='Erro'] {
  color: #c53030;
  background: #fff;
}
